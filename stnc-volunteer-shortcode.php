...
function stncGonulluOl_registration_form()
{
  ...
  /*
		...
		if($registration_enabled) {
			$output = stncGonulluOl_registration_form_fields();
		} else {
...
add_shortcode('stncGonulluOl_RegisterForm', 'stncGonulluOl_registration_form');
// [stncGonulluOl_RegisterForm] --- kullanım örneği


// registration form fields
function stncGonulluOl_registration_form_fields()
{
  ob_start();
...
?>
    ...
    <h3 class="stncGonulluOl_header"><?php _e('Teşekkürler'); ?></h3>
        ...
        <div class="col-md-12 stncVolunteer">
          <div id="templateBody" class="bodyContent">
            <div class="box box-primary">
              ...
              <form role="form"  method="POST" enctype="multipart/form-data" action="<?php the_permalink(); ?>" id="stncGonulluOl_registration_form" class="stncGonulluOl_form">
                <div class="box-body">
...
<script>
    ...
    var myDropzone = new Dropzone(document.body, { // Make the whole body a dropzone
        ...
        addRemoveLinks: true,
        maxFiles:  20, //https://www.infinetsoft.com/Post/How-to-set-limits-for-file-upload-in-dropzone-js/2534#.YIvbS2YzbJ9
  ...
  //  };
                ...
                <div class="box-footer">
                  <input type="hidden" name="stncGonulluOl_register_nonce" value="<?php echo wp_create_nonce('stncGonulluOl-register-nonce'); ?>" />
                  <?php // wp_nonce_field( 'upload_wpcfu_file', 'wpcfu_nonce', true, false ); ?>
                  <input type="hidden" name="stncGonulluOl_user_login" value="<?php echo wp_create_nonce('stncGonulluOl-register-nonce'); ?>" />
...
// register a new user
function stncGonulluOl_add_new_member()
{

  global $wpdb;

  if (isset($_POST["stncGonulluOl_user_login"]) && wp_verify_nonce($_POST['stncGonulluOl_register_nonce'], 'stncGonulluOl-register-nonce')) {
    ...
    if (empty($nameLastname)) {
      ...
      stncGonulluOl_errors()->add('empty', __('Lütfen adınızı giriniz'));
    }


    if (!is_email($mailAdress)) {
      stncGonulluOl_errors()->add('invalidMail', __('Lütfen doğru bir mail adresi giriniz'));
    }


    $errors = stncGonulluOl_errors()->get_error_messages();
